...
public class TaskService {
    ...
    private void validateMultipleChoice(MultipleChoiceTask task) {
        ...
        int correctCount = countCorrectOptions(options);
        ...
        validateOptionTexts(options, task.getStatement());
    }

    private void validateSingleChoice(SingleChoiceTask task) {
        List<Option> options = task.getOptions();
    ...
    private void validateOptionTexts(List<Option> options, String statement) {
    ...
    private void validateTaskOrder(Task task, Course course) {
    ...
    private Course getCourseIfBuilding(Long courseId) {
    ...
    private void checkDuplicateStatement(Task task, Course course) {
    ...
    private void linkOptionsToTask(List<Option> options, Task savedTask) {
        for (Option option : options) {
    ...
    public MultipleChoiceTask saveMultipleChoiceTask(MultipleChoiceTask task) {
        ...
        Course course = getCourseIfBuilding(task.getCourse().getId());
        validateTaskOrder(task, course);
        checkDuplicateStatement(task, course);

        MultipleChoiceTask savedTask = taskRepository.save(task);
        linkOptionsToTask(task.getOptions(), savedTask);

        return savedTask;
    }

    public SingleChoiceTask saveSingleChoiceTask(SingleChoiceTask task) {
        validateSingleChoice(task);
        Course course = getCourseIfBuilding(task.getCourse().getId());
        validateTaskOrder(task, course);
        checkDuplicateStatement(task, course);
